...
if(isset($_SESSION['user']) && isset($_SESSION['pass']) && $_SESSION['admin'] >=$drepturi['ir_log'])
{function generatePages($max, $page, $class)
	...
	function showPage($i, $page, $class, $selected = 'pagination_selected')
	{
		...
		{
			global $name;
			$pageN = replace($_GET['page']);
			$data .= '<span  class="'. $class .'"><a href="?page='.$pageN.'&id='.$i.'" onclick="load()">'. $i .'</a></span> ';
		}
		return $data;
	}
...
if ((!isset($_GET['id'])) || (!is_numeric($_GET['id'])) || ($_GET['id'] < 1)) { $pagenum = 1; }
	...
	$result = mysql_query("SELECT * FROM web.dev_log_retrage");
...
$result2 = mysql_query("select * from web.dev_log_retrage ORDER BY data desc limit ".($page_rows * ($pagenum - 1)).", ".$page_rows.""); 
...
<h4>LISTA ITEME RETRASE</h4>
<table width="100%" border="0">
  <tr class="top">
    <td width="24%" class="iR_stats_level">Data</td>
    <td width="13%" class="iR_stats_level">Admin</td>
    <td width="63%" class="iR_stats_level">Actiune</td>
  </tr>
  <?php 
  while($ir = mysql_fetch_object($result2))
  {
  ?>
  <tr class="top">
    <td class="iR_stats_reset"><?=$ir->data?></td>
    <td class="iR_stats_reset"> <?=$ir->admin?></td>
    <td class="iR_stats_reset"><?=$ir->actiune?></td>
  </tr>
  <?php } ?>
</table>
