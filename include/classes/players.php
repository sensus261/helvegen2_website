...
class paginate
{
	...
	public function dataview($query, $search=NULL)
	{
		...
		if($rowCount>0)
		{
			...
			{
				$number++;
				$row['empire']=get_player_empire($row['account_id']);
				?>
			<tr>
				<th scope="row"><?php print $number; ?></th>
				<td><?php print $row['name']; ?></td>
				<td><img src="<?php print $site_url; ?>images/empire/<?php print $empire=$row['empire']; ?>.jpg" alt="<?php print emire_name($empire); ?>" title="<?php print emire_name($empire); ?>"></td>
